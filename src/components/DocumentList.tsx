...
import { Language, View, Invoice } from '../types';
...
export const DocumentList: React.FC<DocumentListProps> = ({ lang, type }) => {
  const t = translations[lang];
  const isRtl = lang === 'ar';
  const [documents, setDocuments] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [showForm, setShowForm] = useState(false);
  const [editingDoc, setEditingDoc] = useState<Invoice | null>(null);
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, collectionName), where('uid', '==', auth.currentUser.uid));
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const data = snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      })) as any[];
      setDocuments(data.sort((a, b) => b.number.localeCompare(a.number)));
      setLoading(false);
    });
  ...
  const handleDuplicate = async (docToDup: any) => {
    if (!auth.currentUser) return;
    try {
      const { id, number, ...rest } = docToDup;
  ...
  const handleConvertTo = async (document: any, targetType: View, useSameNumber: boolean = false) => {
    if (!auth.currentUser) return;
    try {
      const targetCollection = getCollectionName(targetType);
      const { id, number, status, ...rest } = document;
      ...
      if (!useSameNumber) {
        ...
        const prefixes: Record<string, string> = {
          'invoices': customPrefixes?.invoice || 'FAC',
          'devis': customPrefixes?.devis || 'DEV',
          'bons_commande': customPrefixes?.po || 'BC',
          'bons_livraison': customPrefixes?.dn || 'BL',
          'reçus': customPrefixes?.receipt || 'REC',
          'credit_notes': customPrefixes?.creditNote || 'AV'
        };
        const prefix = prefixes[targetType as string] || 'DOC';
        const now = new Date();
        ...
        if (targetType === 'invoices') {
          ...
          newNumber = `${prefix}-${year}-${randomNum}`;
        } else {
          const randomNum = String(Math.floor(Math.random() * 100)).padStart(2, '0');
          newNumber = `${prefix}-${year}-${month}-${randomNum}`;
        }
      }
      ...
      alert(isRtl ? 'تم التحويل بنجاح' : 'Document converti avec succès');
      setActiveMenu(null);
    } catch (error) {
  ...
  const getStatusLabel = (status: string) => {
    switch (status) {
      case 'finalized': return t.finalized;
  ...
  const filteredDocs = documents.filter(doc => 
    ...
    (doc.clientName || doc.clientId || '').toLowerCase().includes(searchTerm.toLowerCase())
  );

  return (
    <div className="p-4 lg:p-8 max-w-5xl mx-auto">
      {showForm && <DocumentForm lang={lang} type={type} onClose={() => setShowForm(false)} />}
      {editingDoc && <DocumentForm lang={lang} type={type} document={editingDoc} onClose={() => setEditingDoc(null)} />}
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="divide-y divide-gray-50">
          {loading ? (
            ...
            <div className="p-12 text-center text-gray-400">Aucun document trouvé.</div>
          ) : (
            filteredDocs.map((doc) => (
              <div key={doc.id} className={`grid grid-cols-12 gap-4 p-5 items-center hover:bg-gray-50/50 transition-colors ${isRtl ? 'flex-row-reverse' : ''}`}>
                <div className={`col-span-6 md:col-span-4 ${isRtl ? 'text-right' : 'text-left'}`}>
                  ...
                  <div className={`flex items-center gap-2 mt-1 ${isRtl ? 'justify-end' : ''}`}>
                    <span className="text-xs text-gray-500">{doc.clientName || doc.clientId}</span>
                    <span className="text-[10px] text-gray-400">• {formatDate(doc.date)}</span>
                    {doc.clientIce && <span className="text-[10px] text-gray-300 font-mono">ICE: {doc.clientIce}</span>}
                  </div>
                </div>
                
                <div className={`col-span-4 md:col-span-4 ${isRtl ? 'text-right' : 'text-left'}`}>
                  <span className={`inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold uppercase ${getStatusColor(doc.status)}`}>
                    {getStatusLabel(doc.status)}
                  </span>
                </div>

                <div className="hidden md:block md:col-span-3 text-center font-bold text-gray-800">
                  {(doc.total || 0).toFixed(2)} DH
                </div>

                <div className="col-span-2 md:col-span-1 flex items-center justify-end gap-1 relative">
                  <div className="hidden lg:flex items-center gap-1">
                    <button 
                      onClick={() => generateInvoicePDF(doc, lang, type, settings?.branding)}
                    ...
                    {type === 'devis' && (
                      <button 
                        onClick={() => handleConvertTo(doc, 'invoices', true)}
                        className="p-1.5 hover:bg-gray-100 rounded-lg transition-colors text-emerald-500"
                        title={t.convert_to_invoice_same_number}
                      >
                    ...
                    {activeMenu === doc.id && (
                      <>
                        ...
                        >
                          ...
                          <button
                            onClick={() => {
                              generateInvoicePDF(doc, lang, type, settings?.branding);
                              setActiveMenu(null);
                            }}
                            className={`w-full px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-3 ${isRtl ? 'flex-row-reverse text-right' : 'text-left'}`}
                          >
                          ...
                          {type === 'devis' && (
                            <>
                              ...
                              <button
                                onClick={() => handleConvertTo(doc, 'bons_commande', false)}
                                className={`w-full px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-3 ${isRtl ? 'flex-row-reverse text-right' : 'text-left'}`}
                              >
                              ...
                              <button
                                onClick={() => handleConvertTo(doc, 'bons_livraison', false)}
                                className={`w-full px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-3 ${isRtl ? 'flex-row-reverse text-right' : 'text-left'}`}
                              >
                              ...
                              <button
                                onClick={() => handleConvertTo(doc, 'invoices', false)}
                                className={`w-full px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-3 ${isRtl ? 'flex-row-reverse text-right' : 'text-left'}`}
                              >
                              ...
                              >
                                ...
                                {t.convert_to_invoice_same_number}
                              </button>
                            </>
                          )}
                          ...
                          {type === 'bons_livraison' && (
                            <button
                              onClick={() => handleStatusChange(doc.id, 'finalized')}
                              className={`w-full px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-3 ${isRtl ? 'flex-row-reverse text-right' : 'text-left'}`}
                            >
                              <CheckCircle className="w-4 h-4 text-emerald-500" />
                              {t.mark_delivered}
